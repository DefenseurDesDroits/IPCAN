...
if ( ! class_exists( 'SearchAndFilter' ) )
{
	class SearchAndFilter
	{
				...
				<form action="" method="post" class="searchandfilter'.$addclass.'">
					...
					$returnvar .= '
						...
						foreach($fields as $field)
						{
							...
							if($field == "search")
							{
								$returnvar .=  '<li class="form-group  col-md-2 col-sm-6 col-xs-12">';
								if($labels[$i]!="")
								{
									$returnvar .= "<label class='control-label'>".$labels[$i]."</label>";
								}
						...
						$returnvar .=
							...
							<input type="submit" class="search-submit button button--submit" value="'.$submitlabel.'">
						</li>';
		...
		*/
		...
		{
			$returnvar = '<select multiple="multiple" name="'.$args['sf_name'].'[]" class="form-control postform">';
